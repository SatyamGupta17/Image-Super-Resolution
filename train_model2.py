...
def RRCA_block(ip, filters=64, residual_scaling = 0.2):
    residual = ip * residual_scaling  # Add residual scaling
    
    # Enhanced channel attention
    model = Conv2D(filters, (3,3), padding='same', kernel_initializer='he_normal')(ip)
    model = PReLU()(model)
    model = Conv2D(filters, (3,3), padding='same', kernel_initializer='he_normal')(model)
    model = channel_attention(model)
    return add([model, residual])
